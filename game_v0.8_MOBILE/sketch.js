...
let numAliens = 5;
let gameover = false;
let backgroundColor; // We will initialize it in setup
...
function setup() {
  ...
  space_x = 100;
  space_y = 150; // Adjusted to be centered vertically
  ...
  // Create on-screen controls for mobile users
  upButton = createButton('↑');
  upButton.position(50, height - 1);
  upButton.size(60, 60);
  upButton.mousePressed(() => isUpPressed = true);
  upButton.mouseReleased(() => isUpPressed = false);
  upButton.touchStarted(() => isUpPressed = true);
  upButton.touchEnded(() => isUpPressed = false);

  downButton = createButton('↓');
  downButton.position(50, height - -60);
  downButton.size(60, 60);
  downButton.mousePressed(() => isDownPressed = true);
  downButton.mouseReleased(() => isDownPressed = false);
  downButton.touchStarted(() => isDownPressed = true);
  downButton.touchEnded(() => isDownPressed = false);
}

function draw() {
  if (MENU == 0) {
    background(img2); // Display the background image for the menu
    drawMenu();
  } else if (MENU == 1) {
    if (!gameover) {
      updateScore(); // Make sure score is updated every frame
      adjustDifficulty(); // 
      updateBackgroundColor(); // 
      background(backgroundColor); // Use the updated background color
    ...
    background(55, 7, 227); // A specific color background for the instructions
    showInstructions();
  } else if (MENU == 3) {
    background(0); // Black background for the exit screen
    exitGame();
  }
  
  // Call the spaceship update function outside of the gameover condition
  // to ensure it's always processed.
  updateSpaceship();
}

function drawMenu() {
  fill(55, 7, 227);
  rect(165, 50, 350, 75, 50); // Adjust the size and position as needed
  rect(250, 200, 200, 75, 50); // Adjust the size and position as needed
  rect(250, 300, 200, 75, 50); // Adjust the size and position as needed

  textSize(35); // Adjust text size as needed
  fill(255);
  text('START GAME', 180, 100); // Adjust text position as needed
  text('EXIT', 310, 350); // Adjust text position as needed

  textSize(18); // Adjust text size as needed
  text('INSTRUCTIONS', 270, 250); // Adjust text position as needed
}
...
function showInstructions() {
  ...
  fill(255);
  textAlign(CENTER);
  text("Use the UP ARROW key or UP BUTTON to move the spaceship up", width / 2, height / 2 - 20);
  text("Use the DOWN ARROW key or DOWN BUTTON to move the spaceship down", width / 2, height / 2 + 20);
  text("Lose the game to return to the main menu", width / 2, height / 2 + 60);
  textAlign(LEFT); // Reset alignment for other text
}

function exitGame() {
  MENU = 0; // Reset the MENU to show the main menu
}

function showGameOver() {
  fill(255);
  textSize(32); // Adjust as needed
  text("Game Over", width / 2 - 100, height / 2); // Center the text
...
function updateBackgroundColor() {
  // Calculate the score in points
  let points = floor(score / 1000);  // Divide by 1000 if your score is in milliseconds
...
function updateSpaceship() {
  let spaceshipHalfHeight = 10; // Half the height of the spaceship
  if ((keyIsDown(UP_ARROW) || isUpPressed) && space_y > spaceshipHalfHeight) {
    space_y -= 3; // Move up but not off-screen
  }
  if ((keyIsDown(DOWN_ARROW) || isDownPressed) && space_y < height - spaceshipHalfHeight) {
    space_y += 3; // Move down but not off-screen
  }
  drawSpaceship(space_x, space_y); // Draw the spaceship
}
...
function updateAliens() {
  // Define smaller collision boundaries for more precise detection
  let spaceshipCollisionWidth = 20; // Smaller than the visual width
  let spaceshipCollisionHeight = 10; // Smaller than the visual height
  let alienCollisionDiameter;

  for (let i = 0; i < numAliens; i++) {
    drawAlien(i);
    alien_x[i] -= alienSpeed[i];

    // Adjust alien collision diameter based on visual size
    alienCollisionDiameter = alienSizes[i] * 0.8; // Assuming the collision size is smaller
    ...
    // Adjust collision check with smaller collision sizes
    if (collideRectCircle(space_x - spaceshipCollisionWidth / 2, space_y - spaceshipCollisionHeight / 2, spaceshipCollisionWidth, spaceshipCollisionHeight, alien_x[i], alien_y[i], alienCollisionDiameter)) {
      gameover = true;
    }
  }
}
...
function resetGame() {
  ...
  space_y = 200;
  score = 0;
   backgroundColorChanged = false; // Allow color change on next multiple of 25
